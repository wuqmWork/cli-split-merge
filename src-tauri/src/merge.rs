...
pub fn merge_cli_files(app: &AppHandle, request: MergeRequest) -> Result<MergeResult, String> {
    ...
    crate::runtime::log(
        app,
        "info",
        &format!("开始 CLI 合并：{} 个输入文件", request.input_files.len()),
    );
    ...
    let indexed: Vec<IndexedFile> = if parallel {
        ...
        for (i, input) in request.input_files.iter().enumerate() {
            ...
            emit_progress(
                app,
                (i as f64 / (total as f64 * 2.0)) * 100.0,
                i,
                total,
                &name,
                "正在扫描层索引",
    ...
    for file in &indexed {
        for span in &file.layers {
            by_layer
                .entry(LayerKey(span.layer_value))
                .or_default()
                .push(span.clone());
        }
    }
    ...
    let merged_dimension = indexed
        .iter()
        .filter_map(|f| f.dimension)
        .reduce(Dimension::union);
    ...
    let result = (|| -> Result<(), String> {
        ...
        writer
            .write_all(b"$$GEOMETRYSTART\n")
            .map_err(io_err("写入 GEOMETRYSTART"))?;
        if !indexed[0].preamble.is_empty() {
            writer
                .write_all(&indexed[0].preamble)
                .map_err(io_err("写入几何前导内容"))?;
        ...
        let mut sources: Vec<File> = indexed
            ...
            .map(|f| {
                File::open(&f.path)
                    .map_err(|e| format!("无法重新打开 CLI {}：{e}", f.path.display()))
            })
            .collect::<Result<_, _>>()?;
        ...
        for (layer_i, (_key, spans)) in by_layer.iter().enumerate() {
            ...
            for span in spans {
                ...
                if last_progress_emit.elapsed() >= Duration::from_millis(250)
                    || layer_i + 1 == layer_total
                {
                    emit_progress(
                        app,
                        p,
                        completed_files.min(total),
                        total,
                        &current_name,
                        "正在按层合并",
                    );
    ...
    crate::runtime::log(
        ...
        &format!(
            "CLI 合并完成：{} ms，{} 层",
            result.elapsed_ms, result.layer_count
        ),
    );
    if let Ok(payload) = serde_json::to_string(&result) {
        crate::runtime::write_cache(app, "last_merge", &payload);
    }
    Ok(result)
}
...
fn index_file(path: &Path, input_index: usize, capacity: usize) -> Result<IndexedFile, String> {
    ...
    loop {
        ...
        if starts_ascii_ci(token, b"$$LAYER/") {
            ...
            let layer_value =
                parse_layer_value(token).map_err(|e| format!("{}：{}", path.display(), e))?;
            saw_layer = true;
...
fn rewrite_header(
    ...
    loop {
        ...
        let n = reader
            .read_until(b'\n', &mut line)
            .map_err(|e| format!("重写 CLI 头部失败：{e}"))?;
        if n == 0 {
            break;
        }
        let token = trim_ascii(&line);
        ...
        if eq_ascii_ci(token, b"$$HEADEREND") && !inserted_missing {
            if !saw_layers {
                writeln!(&mut out, "$$LAYERS/{layer_count}")
                    .map_err(|e| format!("写入 LAYERS 失败：{e}"))?;
                saw_layers = true;
            }
            if !saw_dimension {
        ...
        if starts_ascii_ci(token, b"$$LAYERS/") {
            writeln!(&mut out, "$$LAYERS/{layer_count}")
                .map_err(|e| format!("重写 LAYERS 失败：{e}"))?;
            saw_layers = true;
        } else if starts_ascii_ci(token, b"$$DIMENSION/") {
            if let Some(d) = dimension {
                write_dimension_line(&mut out, d)?;
...
fn parse_units_line(token: &[u8]) -> Result<f64, String> {
    let slash = token
        .iter()
        .position(|&b| b == b'/')
        .ok_or_else(|| "UNITS 行格式错误".to_string())?;
...
fn parse_dimension_line(token: &[u8]) -> Result<Dimension, String> {
    let slash = token
        .iter()
        .position(|&b| b == b'/')
        .ok_or_else(|| "DIMENSION 行格式错误".to_string())?;
    let payload = trim_ascii(&token[slash + 1..]);
    let text =
        std::str::from_utf8(payload).map_err(|_| "DIMENSION 不是有效 ASCII 数值".to_string())?;
    let values: Vec<f64> = text
        .split(',')
        .map(|v| {
            lexical_core::parse::<f64>(v.trim().as_bytes())
                .map_err(|_| format!("无法解析 DIMENSION 数值：{v}"))
        })
        .collect::<Result<_, _>>()?;
...
fn copy_range(
    ...
    file.seek(SeekFrom::Start(start))
        .map_err(|e| format!("定位源 CLI 失败：{e}"))?;
    let mut remain = end - start;
    while remain > 0 {
        let take = remain.min(buffer.len() as u64) as usize;
        let n = file
            .read(&mut buffer[..take])
            .map_err(|e| format!("读取层数据失败：{e}"))?;
        if n == 0 {
            return Err("源 CLI 在预期层数据结束前意外结束。".to_string());
        }
        writer
            .write_all(&buffer[..n])
            .map_err(|e| format!("写入合并层数据失败：{e}"))?;
        remain -= n as u64;
    }
    Ok(())
}
...
fn display_name(path: &Path) -> String {
    path.file_name()
        .and_then(|s| s.to_str())
        .unwrap_or_else(|| path.to_str().unwrap_or("CLI"))
        .to_string()
}

fn mb_capacity(mb: usize) -> usize {
...
mod tests {
    ...
    fn output_extension_is_added() {
        assert!(normalize_output_path("abc")
            .to_string_lossy()
            .ends_with("abc.cli"));
        assert!(normalize_output_path("abc.CLI")
            .to_string_lossy()
            .ends_with("abc.CLI"));
    }
    ...
    fn units_compare_numerically() {
        assert_eq!(
            parse_units_line(b"$$UNITS/1").unwrap(),
            parse_units_line(b"$$UNITS/1.0").unwrap()
        );
    }
    ...
    fn dimension_union_is_correct() {
        ...
        let u = a.union(b);
        assert_eq!(
            (u.xmin, u.ymin, u.zmin, u.xmax, u.ymax, u.zmax),
            (-1.0, 1.0, 0.0, 10.0, 4.0, 8.0)
        );
    }
